...
try {
  ...
  if (stale.length) {
    console.warn(`[InvestLab] ${stale.length} estrategia(s) obsoleta(s) eliminadas. Re-ejecuta y vuelve a guardar.`);
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetch("/api/backtest", {
      ...
      body: JSON.stringify({ assets:assets.map(a=>({...a})), startDate:start, endDate:end,
    ...
    lastResult = { ...data, assets:assets.map(a=>({...a})),
...
function showResults(r) {
  ...
  const tickers = r.assets.map(a=>`${a.ticker} ${a.allocation}%`).join(" · ");
  ...
  const vl = r.volatility??0, sh = r.sharpe??0, dd = r.maxDrawdown??0;
  ...
  const bm = r.bestMonth,  wm = r.worstMonth;
  const by = r.bestYear,   wy = r.worstYear;
...
function renderAssetBreakdown(breakdown) {
  ...
  if (grid) {
    grid.innerHTML = breakdown.map((a, i) => {
      ...
      const sign  = a.cagr >= 0 ? "+" : "";
      const ddCls = a.maxDrawdown <= -30 ? "red" : a.maxDrawdown <= -15 ? "orange" : "yellow";
      return `
        <div class="asset-card" style="border-top-color:${col}">
          ...
          <div class="ac-row">
            ...
            <span class="ac-val" style="color:${a.cagr>=0?"#22c55e":"#ef4444"}">${sign}${a.cagr.toFixed(2)}%</span>
          </div>
          <div class="ac-row">
            <span class="ac-label">Retorno total</span>
            <span class="ac-val" style="color:${a.totalReturn>=0?"#22c55e":"#ef4444"}">${a.totalReturn>=0?"+":""}${a.totalReturn.toFixed(1)}%</span>
          </div>
          <div class="ac-row">
            <span class="ac-label">Max Drawdown</span>
            <span class="ac-val" style="color:#ef4444">${a.maxDrawdown.toFixed(1)}%</span>
          </div>
          <div class="ac-row">
            <span class="ac-label">Volatilidad</span>
            <span class="ac-val" style="color:#eab308">${a.volatility.toFixed(1)}%</span>
          </div>
          <div class="ac-row">
            <span class="ac-label">Sharpe</span>
            <span class="ac-val" style="color:#6c63ff">${a.sharpe.toFixed(2)}</span>
          </div>
          <div class="ac-row">
            <span class="ac-label">Win Rate</span>
            <span class="ac-val" style="color:#14b8a6">${a.winRate.toFixed(1)}%</span>
          </div>
          <div class="ac-row">
            <span class="ac-label">Contrib. portafolio</span>
            <span class="ac-val" style="color:${a.contribution>=0?"#a855f7":"#ef4444"}">${a.contribution>=0?"+":""}${a.contribution.toFixed(1)}%</span>
          </div>
          ${a.bestYear ? `<div class="ac-row"><span class="ac-label">Mejor año</span><span class="ac-val" style="color:#22c55e">${a.bestYear.year} +${a.bestYear.ret.toFixed(1)}%</span></div>` : ""}
          ${a.worstYear ? `<div class="ac-row"><span class="ac-label">Peor año</span><span class="ac-val" style="color:#ef4444">${a.worstYear.year} ${a.worstYear.ret.toFixed(1)}%</span></div>` : ""}
          ${a.grossDiv > 0 ? `<div class="ac-row"><span class="ac-label">Dividendos brutos</span><span class="ac-val" style="color:#84cc16">${fmt$(a.grossDiv)}</span></div>` : ""}
        </div>`;
    }).join("");
  }
      ...
      return `
        <tr>
          ...
          <td class="${best?"td-best":""}">
            <span style="color:${a.cagr>=0?"#22c55e":"#ef4444"}">${a.cagr>=0?"+":""}${a.cagr.toFixed(2)}%</span>
          </td>
          <td><span style="color:${a.totalReturn>=0?"#22c55e":"#ef4444"}">${a.totalReturn>=0?"+":""}${a.totalReturn.toFixed(1)}%</span></td>
          ...
          <td>
            <span style="color:${a.contribution>=0?"#a855f7":"#ef4444"};font-weight:700">
...
function renderAssetCompChart(breakdown, metric) {
  ...
  const labels = {
    cagr: "CAGR (%)", totalReturn: "Retorno total (%)",
    maxDrawdown: "Max Drawdown (%)", volatility: "Volatilidad (%)",
    sharpe: "Sharpe Ratio", winRate: "Win Rate (%)",
  };

  const data   = breakdown.map(a => a[metric] ?? 0);
  const colors = breakdown.map((a, i) => {
    const v = data[i];
    if (metric === "maxDrawdown") return v < -25 ? "#ef444488" : v < -15 ? "#f9731688" : "#eab30888";
  ...
  assetCompChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          backgroundColor: "#1e2235", borderColor: "#2e3347", borderWidth: 1,
          titleColor: "#e2e8f0", bodyColor: "#94a3b8",
          callbacks: { label: c => `  ${c.dataset.label}: ${c.parsed.x.toFixed(2)}` },
        },
      },
      scales: {
        x: { ticks: { color: "#64748b", callback: v => v+"%" }, grid: { color: "#1a1d2e" } },
  ...
  assetAnnChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { ticks: { color: "#64748b", callback: v => (v>=0?"+":"")+v+"%" }, grid: { color: "#1a1d2e" }, border: { dash: [4,4] } },
      },
    },
  });
}
...
function renderDivPanel(r) {
  ...
  divChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: "#64748b", maxTicksLimit: 16, maxRotation: 45 }, grid: { color: "#1a1d2e" } },
        yBar: {
          position: "left",
          ticks: { color: "#84cc16", callback: v => v >= 1000 ? "$"+(v/1000).toFixed(1)+"k" : "$"+v },
          grid: { color: "#1a1d2e" },
          title: { display: true, text: "Por mes ($)", color: "#64748b", font: { size: 10 } },
        },
        yCum: {
          position: "right",
          ticks: { color: "#22d3ee", callback: v => v >= 1000 ? "$"+(v/1000).toFixed(1)+"k" : "$"+v },
          grid: { display: false },
          title: { display: true, text: "Acumulado ($)", color: "#64748b", font: { size: 10 } },
        },
      },
    },
  });
...
function renderLineChart(strategies) {
  ...
  const hasDivData = showDivs && strategies.some(r =>
  ...
  strategies.forEach((r, si) => {
    ...
    if (showValue) {
      ...
      datasets.push({
        type:"line", label:`Portafolio${tag}`,
        data: r.evolution.map(e=>e.value),
        borderColor:col, backgroundColor:g, borderWidth:2.5,
        fill:true, tension:.35,
        pointRadius:r.evolution.length>60?0:3, pointHoverRadius:5,
        yAxisID: yAx, order: 1,
      });
    }
    if (showNav) datasets.push({
      type:"line", label:`NAV${tag}`,
    ...
    if (showContrib && si===0) datasets.push({
      type:"line", label:"Capital aportado",
    ...
    if (showDivs && si===0) {
      ...
      if (hasDivs_) {
        datasets.push({
          type:"bar", label:"Dividendo neto/mes",
          data: divData,
  ...
  const scales = hasDivData
    ? {
        x:    { ticks:{ color:"#64748b", maxTicksLimit:12, maxRotation:0 }, grid:{ color:"#1a1d2e" } },
        yPort:{ ...scaleY, position:"left",
          title:{ display:true, text:"Portafolio ($)", color:"#64748b", font:{size:10} } },
        yDiv: {
          position:"right",
          ticks:{ color:"#84cc16",
            callback: v => v>=1e3?"$"+(v/1e3).toFixed(1)+"k":"$"+v.toFixed(0) },
          grid:{ display:false },
          title:{ display:true, text:"Dividendo/mes ($)", color:"#84cc16", font:{size:10} },
        },
      }
    : {
        x: { ticks:{ color:"#64748b", maxTicksLimit:12, maxRotation:0 }, grid:{ color:"#1a1d2e" } },
...
function renderAnnualChart(strategies) {
  ...
  annualChart = new Chart(ctx, {
    ...
    options:{
      responsive:true, maintainAspectRatio:false,
      plugins:{
        legend:{ display: strategies.length > 1, labels:{ color:"#94a3b8", font:{size:11} } },
        tooltip:{ backgroundColor:"#1e2235", borderColor:"#2e3347", borderWidth:1,
          titleColor:"#e2e8f0", bodyColor:"#94a3b8",
          callbacks:{ label: c=>`  ${c.dataset.label}: ${c.parsed.y>=0?"+":""}${c.parsed.y?.toFixed(2)}%` } },
      },
      scales:{
...
btnSave.addEventListener("click", () => {
  ...
  persist(); renderSaved(); renderRanking(); hideError();
});
...
btnCompare.addEventListener("click", () => {
  ...
  const vl=best.volatility??0, sh=best.sharpe??0, dd=best.maxDrawdown??0, wr=best.winRate??0;
  ...
  set("sBestMonth",  best.bestMonth?.ret  != null ? `${fmtMonth(best.bestMonth.date)}  +${Number(best.bestMonth.ret).toFixed(1)}%`  : staleHint);
  set("sWorstMonth", best.worstMonth?.ret != null ? `${fmtMonth(best.worstMonth.date)}  ${Number(best.worstMonth.ret).toFixed(1)}%` : staleHint);
  set("sBestYear",   best.bestYear?.ret   != null ? `${best.bestYear.year}  +${Number(best.bestYear.ret).toFixed(1)}%`   : staleHint);
  set("sWorstYear",  best.worstYear?.ret  != null ? `${best.worstYear.year}  ${Number(best.worstYear.ret).toFixed(1)}%`  : staleHint);
  if (stale) showError("⚠️ Estrategias guardadas con versión anterior. Re-ejecútalas y vuelve a guardarlas para ver todas las métricas.");
...
function renderCompareTable(strategies) {
  ...
  strategies.forEach((s, i) => {
    const col = PALETTE[i%PALETTE.length];
    const by  = s.bestYear,  wy = s.worstYear;
    ...
    tbody.innerHTML += `
      <tr>
        <td><span class="tbl-dot" style="background:${col}"></span>${s.name}</td>
        ...
        <td class="td-green">${by?.ret != null ? `${by.year} +${Number(by.ret).toFixed(1)}%` : "—"}</td>
        <td class="td-red">${wy?.ret  != null ? `${wy.year} ${Number(wy.ret).toFixed(1)}%`  : "—"}</td>
      </tr>`;
  });
}
...
function renderRanking() {
  ...
  if (savedStrategies.length < 2) { wrap.style.display="none"; return; }
  wrap.style.display = "block";

  const sel    = document.getElementById("rankBy").value;
  const sorted = [...savedStrategies].sort((a,b) => {
    const av = a[sel]??0, bv = b[sel]??0;
...
window.delStrategy  = i  => { savedStrategies.splice(i,1); persist(); renderSaved(); renderRanking(); };
...
const buildName = (list, s, e) =>
  list.map(a=>`${a.ticker}${list.length>1?" "+a.allocation+"%":""}`).join("+")+` · ${s.slice(0,4)}–${e.slice(0,4)}`;
